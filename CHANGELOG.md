...
- **新增界面预览**：README 顶部与「界面预览」章节加入 4 张实机截图
  （服务设置 / 账户与权限 / 服务状态 / 内核管理），取自 ImmortalWrt SNAPSHOT
  实机（LuCI Master + Aurora 主题），存放于 `docs/screenshots/`。
  截图仅做最小限度脱敏：账户密码列掩码、设备管理地址替换为示例值；
  路径、端口、账户名均为包默认值，保持界面原貌。
